...
runners.map((cap) => {
    return firstNamesAllCaps.push(cap.first_name.toUpperCase());
});
console.log(firstNamesAllCaps);

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.
//***I had it working with a if/else loop, but that defeated the filter function, it was really long winded. So I hit the google, and found this. It's way less verbose and very effective ****/
...
let ticketPriceTotal = runners.reduce((runningTotal, cash) => {
    return cash.donation + runningTotal
}, 0)
console.log('The total amount of donations: $' + ticketPriceTotal);
...
runners.forEach((info) => {
    return emailList.push(info.first_name + " " + info.last_name + ": " + info.email)
})
console.log(emailList);
// Problem 2
//Big Donors
let bigDonor = [];
bigDonor = runners.filter(money => money.donation >= 200)
console.log(bigDonor);

// Problem 3
//Isolating one company's runners
let team = [];
team = runners.filter(compName => compName.company_name === 'Skinix')
console.log(team);